...
class MpesaService:
    ...
    def get_access_token(self):
        ...
        try:
            ...
            if hasattr(e, 'response') and e.response is not None:
                logger.error(f"Failed to get access token. Status: {e.response.status_code}, Response: {e.response.text}")
            else:
                logger.error(f"Failed to get access token: {str(e)}")
            
            # Fallback to mock if sandbox
            if self.env == 'sandbox':
                logger.warning("Falling back to MOCK token due to connection error.")
    ...
    def stk_push(self, phone_number, amount, account_reference, transaction_desc):
        ...
        payload = {
            ...
            "Amount": int(float(amount)), # Amount must be integer
            "PartyA": phone_number,
